...
// method mengembalikan nilai (tidak menggunakan void) --> FUNCTION
	...
	public void selectionSort() { //Membuat method selectionSort untuk mengurutkan data dari NIM yang terkecil
		int out, in, min;
		for(out = 0; out < jml_data - 1; out++) { // mengurutkan data dari yang terkecil ke terbesar
			min = out; // data terkecil adalah data pertama
			for(in = out + 1; in < jml_data; in++) { // mencari data terkecil
				if( dt[in].getNim().compareTo(dt[min].getNim()) < 0 ){ // membandingkan data pada array dengan yang dicari
					min = in; // jika lebih kecil maka data terkecil adalah data yang dicari
				}
			}
			if (min != out) { // jika data terkecil bukan data pertama maka tukar data
				mahasiswa temp = dt[out];
				dt[out] = dt[min];
				dt[min] = temp;
			}	
		}
...
//----------------------------------------------------------------------
		...
		do {
			...
			if (menu==2) {
				...
				md.selectionSort(); // memanggil method selectionSort pada class metod
